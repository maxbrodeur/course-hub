...
class Editor(QMainWindow):
	...
	def createProjectButtons(self):
		...
		prevButton.clicked.connect(lambda :self.projectFrameGo(False))
		nextButton.clicked.connect(lambda :self.projectFrameGo(True))
	...
	def projectFrameGo(self, next_or_prev):
		if not self.hasSavedOpen:
			return
		...
		num = self.lastProjectLength
		pos = self.projectPlayer.get_position() * num
		if pos < 0:
			new_pos = 0
		elif pos > num:
			new_pos = num
		...
		self.projectPlayer.set_position(new_pos / self.lastProjectLength)
		self.projectSlider.setValue(new_pos)
		if new_pos <= self.frames_list.count():
			self.frames_list.setCurrentRow(new_pos)
